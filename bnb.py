...
class BernoulliNaiveBayes(object):
    ...
    def fit(self, X, Y, all_classes):
        ...
        print(self.class_name, class0, classn)
        print("FUK OFF", Y.shape[0])
    ...
    def _bernoulli_from_features(self, x, p):
        return np.product(np.where(x == 1, p, 1 - p))
...
def main():
    ...
    for comment, features in zip(X_raw, X):
        if np.all(features == 0):
            print("Comment is fucked: ", comment)
